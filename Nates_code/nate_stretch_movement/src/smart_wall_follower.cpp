...
{
    //setting up node
    ros::init(argc, argv, "smart_wall_follower_node");
    ros::NodeHandle n;
...
void thinker(const sensor_msgs::LaserScan::ConstPtr &scan)
{
    ...
    while(i < 800)
    {
        if(scan->ranges[i] < 0.45) wall_in_frr = true;
        i++;
    }
    ...
    while(i < 1000)
    {
        if(scan->ranges[i] < 0.4) wall_in_front_right = true;
        i++;
    }
    ...
    while(i < 1400)
    {
        if(scan->ranges[i] < 0.45) wall_in_ffr = true;
        i++;
    }
...
void right_wall_follower(const sensor_msgs::LaserScan::ConstPtr &scan)
{
    ...
    switch(wall_locations)
    {
        ...
        case 1: //wall on left
            ...
            break;
        case 2: //wall in front FRONT
            ROS_INFO("case 2");
            if(!wall_in_ffr && !wall_in_front_right && !wall_in_frr)
            {
                motorizer.angular.z = -0.7;
                motorizer.linear.x = 0.2;
            }
            else if(!wall_in_front_right && !wall_in_frr)
            {
                motorizer.angular.z = -0.8;
                motorizer.linear.x = 0.1;
            }
            else if (!wall_in_front_right && !wall_in_ffr)
            {
                motorizer.angular.z = -0.5;
                motorizer.linear.x = 0.2;
            }
        ...
        case 3: //wall in front and on left FRONT
            ROS_INFO("case 3");
            motorizer.angular.z = 0.5;
            break;
        case 4: //wall on right
            ...
            if(!wall_in_ffr && !wall_in_front_right && !wall_in_frr)
            {   
                ROS_INFO("case 4: Front right completely clear");
                motorizer.angular.z = -0.8;
                motorizer.linear.x = 0.5;
            }
            else if(!wall_in_frr && !wall_in_front_right)
            {   
                ROS_INFO("case 4: Front right somewhat clear");
                motorizer.angular.z = -0.7;
                motorizer.linear.x = 0.5;
            }
            else if(!wall_in_frr)
            {   
                ROS_INFO("case 4: Front right nearly clear");
                motorizer.angular.z = -0.3;
                motorizer.linear.x = 0.5;
            }
            else
        ...
        case 5: //wall on left and right
            ...
            motorizer.linear.x = 0.5;
            break;
        case 6: //wall front and right FRONT
            ROS_INFO("case 6");
            motorizer.angular.z = 0.5;
            break;
        case 7: //walls front, left, and right FRONT
            ROS_INFO("case 7");
            motorizer.angular.z = 0.5;
            break;
    }
