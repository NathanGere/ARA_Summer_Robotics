...
int main(int argc, char** argv)
{
    ...
    nh.param<std::string>("laser_frame", laser_frame, "/base_link"); //"/base_laser_link"
    nh.param<std::string>("topic_name", topic_name, "/scan"); //"robot/base_scan"
    ...
    while(nh.ok())
    {
        ...
        transformPrinter(listener, closest_wall_frame_left, "on the left");
        rate.sleep();
    }
    return 0;
}
