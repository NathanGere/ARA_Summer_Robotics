...
{
    //setting up node
    ros::init(argc, argv, "right_turn_only_wall_follower_node");
    ros::NodeHandle n;
...
void thinker(const sensor_msgs::LaserScan::ConstPtr &scan)
{
    ...
    while(i < 47)
    {
        if(scan->ranges[i] < 0.4) wall_on_left = true;
        i++;
    }
    ...
    while(i < 890)
    {   
        if(scan->ranges[i] < 0.4) wall_on_right = true;
        i++;
    }
    ...
    while(i < 1640)
    {
        ...
        if(scan->ranges[i] < 0.3) close_on_front = true;
        i++;
    }
